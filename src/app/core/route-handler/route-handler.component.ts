...
export class RouteHandlerComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    const ocorrenciaViagem = new SidenavItem({
      name: 'Ocorrência Viagem',
      icon: 'library_books',
      route: '/ocorrencia-viagem',
      subItems: [ ],
      position: 1
    });
    ...
    this.store.dispatch(new fromSidenav.AddSidenavItemAction(ocorrenciaViagem));
